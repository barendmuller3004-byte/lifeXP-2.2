...
function defaultState(){
  ...
  return {
    ...
    settings: {
      ...
      appearance: { theme:'cozy', accent:'#D99A3F', preset:'honey', cornerStyle:'soft', glassIntensity:0, transparency:0, glowIntensity:130, iconStyle:'outline', navIconsOnly:false, brightness:100, bgStyle:'none', bgOpacity:8, bgImageUrl:'', custom:{ bg:'#0B0F17', card:'#141B28', card2:'#1B2434', text:'#F2F3F7' } },
      animations: {
        mode:'standard',
...
let qaState = { due:null, recurrence:'none', tag:null };
let qaItems = [];
let qaAnalyses = [];
function openQuickAdd(){
  ...
  qaState = { due:null, recurrence:'none', tag:null };
  qaItems = []; qaAnalyses = [];
  document.querySelectorAll('#qaChips .qa-chip').forEach(b=>b.classList.remove('active'));
...
function submitQuickAdd(){
  ...
  let dueDate = qaState.due==='today' ? todayISO() : qaState.due==='tomorrow' ? tomorrowISO() : null;
  items.forEach((title,i)=>{
    ...
    const t = makeTaskFromTitle(title, { starredGoal:(noGoalYet && i===0), dueDate:taskDue, tag:qaState.tag, recurringTemplateId:templateId });
...
function renderBrainDumpResults(){
  ...
  let html = '<div class="bd-summary-line">Found '+bdGoals.length+' goal'+(bdGoals.length>1?'s':'')+', '+totalTasks+' tasks. Uncheck anything you don\'t want.</div>';
...
function addAllExtractedGoals(){
  ...
  bdGoals.forEach(g=>{
    g.tasks.filter(t=>t.checked).forEach(t=>{
      const task = makeTaskFromTitle(t.title, { projectTitle: g.goalTitle, starredGoal:(noGoalYetBefore && !firstAdded) });
  ...
  showToast('+'+added+' tasks added from '+bdGoals.length+' goals');
}
...
function heroTargetTask(){
  return state.tasks.find(t=>!t.completed && t.starredGoal) ||
    state.taskOrder.map(id=>state.tasks.find(t=>t.id===id)).find(t=>t && !t.completed) || null;
}
...
function renderCourt(){
  ...
  wrap.innerHTML = html;
  const navBtn = document.getElementById('courtNavBtn');
  if(navBtn) navBtn.classList.toggle('court-alert', open.length>0);
}
...
function iconEdit(){ return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 013 3L7 19l-4 1 1-4z"/></svg>'; }
function iconArchive(){ return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="4" rx="1"/><path d="M5 8v11a1 1 0 001 1h12a1 1 0 001-1V8"/><path d="M10 13h4"/></svg>'; }
function iconTrash(){ return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M8 6V4a1 1 0 011-1h6a1 1 0 011 1v2m2 0v14a1 1 0 01-1 1H7a1 1 0 01-1-1V6z"/><path d="M10 11v6M14 11v6"/></svg>'; }
function iconGrip(){ return '<svg viewBox="0 0 24 24" width="13" height="13" fill="currentColor"><circle cx="8" cy="6" r="1.4"/><circle cx="16" cy="6" r="1.4"/><circle cx="8" cy="12" r="1.4"/><circle cx="16" cy="12" r="1.4"/><circle cx="8" cy="18" r="1.4"/><circle cx="16" cy="18" r="1.4"/></svg>'; }
/* ============ Google Sign-In ============
...
function initGoogleSignIn(){
  ...
  if(GOOGLE_CLIENT_ID.indexOf('YOUR_GOOGLE_OAUTH_CLIENT_ID')===0){
    btnEl.innerHTML = '<div class="qa-hint" style="margin:0;">Add your own Google OAuth Client ID in the code (search for GOOGLE_CLIENT_ID) and host this file on a real domain to enable sign-in.</div>';
    return;
  }
  try{
...
function handleGoogleCredential(response){
  try{
    ...
    renderAccount();
    render();
  }catch(e){ console.error('Failed to read Google credential:', e); }
}
...
const ONBOARD_STEPS = [
  ...
  { icon:'🤖', title:'AI is optional, not required', body:'Add a free Gemini key in <b>Settings → AI</b> to unlock smart scoring, daily planning, and insights. Everything still works fine without one — a solid built-in estimate takes over instead.' },
...
function applyBrightness(){
  ...
  mainEl.style.filter = val===100 ? '' : 'brightness('+(val/100).toFixed(2)+')';
}
...
function renderSettingsPage(){
  ...
  groupHtml.appearance = sgroup('appearance','🎨','Appearance',
    ...
    ((s.appearance&&s.appearance.bgStyle&&s.appearance.bgStyle!=='none') ? (
      '<div class="toggle-row" style="margin-top:6px;"><span class="toggle-row-label">Background opacity</span><input type="range" min="2" max="30" value="'+(s.appearance&&s.appearance.bgOpacity!=null?Number(s.appearance.bgOpacity):8)+'" oninput="setPref(\'appearance.bgOpacity\', this.value)" style="width:120px"></div>' +
      (s.appearance.bgStyle==='image' ? '<input type="text" class="settings-search-input" style="margin:8px 0 0;" placeholder="Paste an image URL…" value="'+escapeHtml(s.appearance.bgImageUrl||'')+'" oninput="setPref(\'appearance.bgImageUrl\', this.value)">' : '')
    ) : '') +
  ...
  groupHtml.notifications = sgroup('notifications','🔔','Notifications',
    ...
    '<div class="info-box">Notifications fire only while this tab is open in your browser — this is a local app with no push server.</div>'
  );
...
/* ============ Swipe between pages ============ */
const NAV_ORDER = ['home','skills','challenges','rewards','court'];
...
function renderTasks(){
  ...
  let pending = state.taskOrder.map(id=>state.tasks.find(t=>t.id===id)).filter(t=>t && !t.completed);
...
function dayInfo(dayIso){
  ...
  const pendingDue = state.tasks.filter(t=>!t.completed && t.dueDate===dayIso);
